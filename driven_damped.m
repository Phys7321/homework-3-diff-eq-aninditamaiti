function [period, sol,te] = driven_damped(R, gamma ,theta0,thetad0,grph, omegap, A) 
...
r0 = [theta0 thetad0];
[t,w, te] = ode45(@proj,tspan,r0,opts,g,R, gamma, omegap, A);
sol = [t,w];

ind= find(w(:,2).*circshift(w(:,2), [-1 0]) <= 0);
maximum=length(ind); % to neglect first transient part
ind = ind(7:maximum);
...
%-------------------------------------------
%
function rdot = proj(t,r,g,R, gamma, omegap, A)
    rdot = [r(2); (-2*gamma*r(2)- g*sin(r(1))/R) + A*cos(omegap*t)];
end
