...
E1_total = 0.5*m*R^2*(r1dot.^2) + 0.5*m*R^2*( w2*r1.^2);

[T2, y2] = pendulum1(R,theta0_2, dtheta0, 1);
t2 = y2(:,1);
r2 = y2(:,2);  % theta
r2dot = y2(:,3); % d(theta)/dt
E2_total = 0.5*m*R^2*(r2dot.^2) + 0.5*m*R^2*( w2*r2.^2);

[T3, y3] = pendulum1(R,theta0_3, dtheta0, 1);
t3 = y3(:,1);
r3 = y3(:,2);  % theta
r3dot = y3(:,3); % d(theta)/dt
E3_total = 0.5*m*R^2*(r3dot.^2) + 0.5*m*R^2*( w2*r3.^2);

[T4, y4] = pendulum1(R,theta0_4, dtheta0, 1);
t4 = y4(:,1);
r4 = y4(:,2);  % theta
r4dot = y4(:,3); % d(theta)/dt
E4_total = 0.5*m*R^2*(r4dot.^2) + 0.5*m*R^2*( w2*r4.^2);

[T5, y5] = pendulum1(R,theta0_5, dtheta0, 1);
t5 = y5(:,1);
r5 = y5(:,2);  % theta
r5dot = y5(:,3); % d(theta)/dt
E5_total = 0.5*m*R^2*(r5dot.^2) + 0.5*m*R^2*( w2*r5.^2);

figure(1);
plot(t1,r1,'r-',t1,r1dot,'b-',t1,E1_total, 'g-');
legend('position for theta0=0.1','velocity for theta0=0.1','Total energy');
title('Theta_0 = 0.1')

figure(2);
plot(t2,r2,'r-',t2,r2dot,'b-',t2,E2_total, 'g-');
legend('position for theta0=0.2','velocity for theta0=0.2','Total energy');
title('Theta_0 = 0.2')

figure(3);
plot(t3,r3,'r-',t3,r3dot,'b-',t3,E3_total, 'g-');
legend('position for theta0=0.4','velocity for theta0=0.4','Total energy');
title('Theta_0 = 0.4')

figure(4);
plot(t4,r4,'r-',t4,r4dot,'b-',t4,E4_total, 'g-');
legend('position for theta0=0.8','velocity for theta0=0.8','Total energy');
title('Theta_0 = 0.8')

figure(5);
plot(t5,r5,'r-',t5,r5dot,'b-',t5,E5_total, 'g-');
legend('position for theta0=1','velocity for theta0=1','Total energy');
title('Theta_0 = 1')
